...
(** Keys are generated by sampling in this distribution *)
type keypair = pkey * skey.
op keypairs: { keypair distr | is_lossless keypairs } as dkp_ll.
...
module RSAEMP_game(Adv: RSAEMP_adv) = {
  proc main() = {
    ...
    (* The Charmichael function (lambda) on n defines the smallest integer k such that for all element a
     * coprime with n, a^k mod n = 1. In the case of an RSA modulus it is lcm(p-1, q-1). *)
...
(* Abstract distribution over Zn* (where n is given as a parameter) *)
op Zn_star (n: int): { int distr | is_lossless Zn_star } as zn_ll.
axiom Zn_is_correct n x: 
support (Zn_star n) x => 0 <= x /\ x < n /\ coprime x n.


module Factoring_using_lambda = {

  proc split_s_t(L: int): int * int = {
    (* return (s, t) such that L = t*2^s and t is odd *) 
    var s: int;
    var t: int;
    s <- 0;
    t <- L;
  ...
  proc do_one_trial(n: int, t: int): int option * int = {
      var x: int;
      var y: int option;
      var success: bool;
      success <- false;
  ...
  proc factor(n: int, lambda: int): int * int = {
    var s: int;
    var t: int;
    var x: int;
    var y: int option;
    ...
    success <- false;
    (s, t) <@ split_s_t(lambda);

    while (success = false) {
      (y, x) <@ do_one_trial(n, t);
...
module RSAFP_using_RSAEMP(A: RSAEMP_adv): RSAFP_adv = {
  proc factorize(n: int): int * int = {
    var p: int;
    var q: int;
    var z: int;
    ...
    (p, q) <@ Factoring_using_lambda.factor(n, z);
    return (p,q);
  }
}.
...
(* EMP ==> RSAFP*)
(* Not implemented since the reduction isn't either *)
lemma RSAEMP_to_RSAFP_red(A <: RSAEMP_adv) &m : Pr[RSAEMP_game(A).main() @ &m : res] <= Pr[RSAFP_game(RSAFP_using_RSAEMP(A)).main() @ &m : res].
    admit.
qed.
...
(* RSAGOP ==> RSAKRP *)
lemma RSAGOP_to_RSAKRP_red(A <: RSAGOP_adv) &m : Pr[RSAGOP_game(A).main() @ &m : res] <= Pr[RSAKRP_game(RSAKRP_using_RSAGOP(A)).main() @ &m : res].
byequiv=>//.
proc; inline *.
wp.
call (_: true).
...
split.
smt.
move=> ? result_L result_R A_L A_R eq.
move=>res_L_is_phi.
have res_is_phi : result_R = (s_p SK - 1)*(s_q SK - 1) by smt.
rewrite res_is_phi.
move=> @/invm.
(* Here we should first prove the uniqueness of the inverse *)
admit.
qed.
